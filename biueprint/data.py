...
def data_v1_list():
    if request.method == 'GET':     # 获取全部数据
        ...
        return return_json(code=code.Add_Update, data=tmp_data.to_json())
    else:
...
def data_v1_search():
    ...
    if search_type == 'url':    # 搜索url
        ...
        if not tmp_data:
            return return_json(code=code.NotFound, mess=f"查询无数据:{keyword}")
        ...
        return return_json(code=code.NotFound, data=False, mess=f"查询不存在:{keyword}")
...
def lishi_v1_list():
    if request.method == 'GET':
        ...
        if History.query.filter(History.url.contains(data["url"])).all():
            return return_json(code=code.OK, data=data, mess="数据库中已存在")
